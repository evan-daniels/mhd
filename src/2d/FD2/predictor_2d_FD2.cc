...
void CENTPACK::predictor_2d_FD2(const doublearray3d& un, doublearray3d& u_star, const double& alpha, const doublearray1d& lambda, const doublearray1d& mu, const doublearray1d& parameters)
{
	...
	for (j = 0; j < J+4; j++)
	{
		for (k = 0; k < K+4; k++)
		{
			...
			flux_x(u_vector, parameters, fluxx);
			flux_y(u_vector, parameters, fluxy);
...
//calculate predicted values
  	...
  	for(l = 0; l < L; l++)
	{
		...
		{
			for (k = 1; k < K+3; k++)
				u_star(j,k,l) = un(j,k,l) - 0.5*(lambda(j)*f_prime(j,k,l) + mu(k)*g_qrime(j,k,l));
		}
	}
}
