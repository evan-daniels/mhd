...
//
// Function called by centpack_main_2d_SD2()
//
...
//
//     u3_id_n -- velocity in z-direction
//
//     b1_id_n -- magnetic field z-direction
//
//     b2_id_n -- magnetic field x-direction
//
//     b3_id_n -- magnetic field z-direction
//
//     p_id_n -- pressure
//
//     t_n -- time
...
void CENTPACK::writeout(const doublearray3d& un, const double& t, const doublearray1d& parameters, const long& n, const int& id, const int& p)
{
	...
	for (j = 0; j < J; j++)
	{
	  for (k = 0; k < K; k++)
	  {
		  ...
		  b2(j,k) = un(j+2,k+2,5);
		  b3(j,k) = un(j+2,k+2,6);
		  press(j,k) = (gamma - 1.0)*(un(j+2,k+2,7) - 0.5*((pow(un(j+2,k+2,1),2.0) + pow(un(j+2,k+2,2),2.0) + pow(un(j+2,k+2,3),2.0))/un(j+2,k+2,0)) - 0.5* (pow(un(j+2,k+2,4),2.0) + pow(un(j+2,k+2,5),2.0) + pow(un(j+2,k+2,6),2.0)));
	  }
	}
	...
	sprintf(u3_file, "u3_files/u3_p%0.3d_%0.3ld", id, n);
	sprintf(b1_file, "b1_files/b1_p%0.3d_%0.3ld", id, n);
	sprintf(b2_file, "b2_files/b2_p%0.3d_%0.3ld", id, n);
	sprintf(b3_file, "b3_files/b3_p%0.3d_%0.3ld", id, n);
	sprintf(p_file, "p_files/p_p%0.3d_%0.3ld", id, n);
	sprintf(t_file, "t_files/t_p%0.3d_%0.3ld", id, n);
